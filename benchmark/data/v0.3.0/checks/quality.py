...
def quality_checks():
    quality = read(HERE / 'comet.json')
    native_dir = HERE / 'native'
    native = read(native_dir / 'manifest.json')
    previous = read(HERE / 'app-scores.json')
    app_dir = HERE / 'app'
    # Runs whose translations came out identical share one stored export;
    # every record carries the sha256 that locates it.
    stored = {sha(p): p for p in native_dir.glob('*.json') if p.name != 'manifest.json'}
    ...
    for lang, digest in native['corpus_sha256'].items():
        assert digest == sha(A.corpus(lang))
    for name, digest in native['harness_files'].items():
        assert digest == sha(A.collector(f'quality/{name}'))
    ...
    for job in quality['runs']:
        ...
        path = (stored[job['source_sha256']] if job['label'].startswith('native-')
                else app_dir / Path(job['source_file']).name)
