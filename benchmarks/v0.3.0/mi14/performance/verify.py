...
def run_checker(kind, *args):
    ...
    with tempfile.TemporaryDirectory(prefix='foxlet-archive-check-') as temp:
        root = Path(temp)
        shutil.copyfile(HERE / 'collector/bench_device.py', root / 'bench_device.py')
        checker = root / kind
        checker.mkdir()
        if kind == 'app':
            for source, target in [('app-check.py', 'check.py'), ('app-run.py', 'run.py')]:
                shutil.copyfile(HERE / 'collector' / source, checker / target)
        else:
...
def app_checks():
    ...
    previous_app = read(previous / 'app-reference/results.json')
