...
layouts = {
  ...
  'mi14': dict(base='/data/local/tmp/bergamot', cpus=[2, 7], mask='7c', soc='Snapdragon 8 Gen 3', i8mm=True, gate_khz=2630400, cooldown=8,
               cfg1024='config-mbw1024.yml', jaen1024='config-jaen.yml', cfg512p='config-mbw512on.yml', jaen512='config-jaen512.yml'),
}
...
def gate():
    """Mi 14 lowers scaling_max_freq under sustained load and recovers within a minute or two when idle;
    wait (up to 180 s) for every measured core to be back at the gate value so each cell starts from the same cap."""
...
report = {'measured_at': time.strftime('%Y-%m-%dT%H:%M:%S%z'), 'device': {'model': shell('getprop ro.product.model'), 'android': shell('getprop ro.build.version.release'), 'soc': L['soc'], 'i8mm': L['i8mm']},
  ...
  'protocol': {'affinity': L['mask'], 'rounds': 3, 'version_order': ['AB', 'BA', 'AB'], 'passes_per_process': 3, 'cache_size': 0, 'cooldown_seconds': COOLDOWN, 'gate_khz': GATE,
