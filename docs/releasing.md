...
记录日期：2026-09-13。API 统一实现为 `fa351ed`，基于 `e699e1c`，位于 `codex/unified-api`。下表保存推送前的预检状态，不随工作流推进更新。最终发布状态与设备结果见 [v0.4.0 Release](https://github.com/yinvoke/foxlet-translate/releases/tag/v0.4.0)。
...
源码按下方流程推送。正式发布要求最终提交的 CI 和当次 CI 构件的设备验收通过。
