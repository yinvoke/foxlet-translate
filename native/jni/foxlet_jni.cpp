// JNI adapter for io.github.yinvoker.foxlet.NativeBridge.
// Each call submits a batch and waits for all results.
//
...
//     touch this handle. No worker dispatch or per-worker replicas are required.
...
Java_io_github_yinvoker_foxlet_NativeBridge_createService(JNIEnv *env, jobject, jint workers, jint cacheSize) {
  try {
    ...
    const bool blocking = numWorkers <= 1;
    {
      ruy::Context probe;
      ruy::CpuInfo cpuInfo;
      // Cache sizes 32768/524288 identify ruy's fallback cache parameters when
      // cpuinfo cannot read topology. Log the values observed in this app process.
      // smmla reports i8mm availability; otherwise ruy selects SDOT or NEON.
      // mode/workers record the selected service and inference thread count.
...
Java_io_github_yinvoker_foxlet_NativeBridge_loadModel(JNIEnv *env, jobject, jlong service, jstring configYaml,
  ...
  try {
    ...
    // Load and validate model, shortlist and vocabulary bytes from YAML paths
    // before constructing model replicas. Optional Kotlin prefix bytes replace
    // the bundle's custom rule data; built-in rules remain compiled native data.
