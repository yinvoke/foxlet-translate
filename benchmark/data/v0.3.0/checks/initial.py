...
REPORT = A.path('mi14/initial/results.json')
CHECK_PATH = A.collector('initial/tools/version-bench/check.py')
# Collector programs are stored once under benchmark/data/shared/collectors/;
# import them under the names the archived checker expects.
sys.path.insert(0, str(CHECK_PATH.parent))
import bench_device  # noqa: F401  (check.py imports it by name)
from suite import load_suite

_spec = importlib.util.spec_from_file_location('check', CHECK_PATH)
...
def analyze(path=REPORT):
    ...
    for name, expected in raw['collector_files_sha256'].items():
        assert hashlib.sha256(A.collector(f'initial/{name}').read_bytes()).hexdigest() == expected, name
    assert hashlib.sha256(A.collector('initial/tools/version-bench/main.cpp').read_bytes()).hexdigest() == raw['harness_sha256']
    ...
    verdict = subprocess.run([sys.executable, str(CHECK_PATH),
...
if __name__ == '__main__':
    ...
    parser.add_argument('report', nargs='?', type=Path, default=REPORT)
