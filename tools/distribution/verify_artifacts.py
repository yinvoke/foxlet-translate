#!/usr/bin/env python3
"""Reject incomplete notices, unexpected sentence-prefix resources, and evaluation data in public binaries."""
import argparse
import io
...
def inspect(path):
    ...
    if any('nonbreaking_prefixes/' in name for name in contents):
        raise ValueError(f'{path}: legacy Moses prefix resources found')
    if any('sentence_prefixes/' in name for name in contents):
        raise ValueError(f'{path}: sentence prefixes must be embedded as code, not resource files')
    ...
    for license_marker in [b'Mozilla Public License', b'Apache License', b'MIT License', b'LLVM Exceptions', b'UNICODE LICENSE V3']:
        if license_marker not in notice: raise ValueError(f'{path}: missing license {license_marker}')
    snippets = []
    for lang in ['eng', 'jpn']:
        snippets.extend((ROOT / f'benchmark/app/src/main/assets/bench/{lang}.txt').read_text().splitlines()[:3])
    ...
    print(f'PASS {path}: notices, source offer, no external prefix tables, no benchmark corpus')
