...
0.4.0 以 `Foxlet.create`、`foxlet.models`、`foxlet.translator` 和 `shutdown` 为应用入口。旧的 `BergamotEngine`、`FoxletEngine`、`EngineConfig`、`ModelCatalog`、`ThreadTuning`、`NonbreakingPrefixes` 类及嵌套类型不再出现在最终 AAR 中；本次不提供源码或二进制兼容层。
