...
def render(config,data,en=False):
    ...
    if initial:
        ...
        before, after = initial['before'], initial['after']
        lines.append((f'In the [historical Xiaomi 14 native comparison]({link}), **{before} → {after}**, both built in Release mode, showed roughly **{speed:.0f}% higher single-thread first-pass throughput** and **{memory:.0f}% lower peak RSS**, including changes to default parameters. This descriptive comparison skipped the frequency start gate; it does not measure the full improvement from the early Android app to the current SDK or the advantage over ML Kit. Current-version app measurements are reported below and in the [version report]({report}).' if en else f'[小米 14 历史原生对照]({link})中，**{before} → {after}** 均使用 Release 构建，默认单线程首次翻译速度约提升 **{speed:.0f}%**、峰值 RSS 约降低 **{memory:.0f}%**，其中包含默认参数变化。该组为跳过频率起跑门槛的参考测量；上述数字不表示早期 Android App 到当前 SDK 的全部改善，也不表示相对 ML Kit 的提升。当前版本的 App 实测见下表及[版本报告]({report})。'))
