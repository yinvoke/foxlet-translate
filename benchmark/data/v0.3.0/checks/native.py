"""Verify native/app performance reports and their translation score linkage."""
import hashlib
import json
...
REPO = CHECKS.parents[3]
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / 'shared'))
from archive import open_archive

A = open_archive(CHECKS.parent)
...
def run_checker(kind, *args):
    # Replay the recorded package checks independently of later SDK renames.
    with tempfile.TemporaryDirectory(prefix='foxlet-archive-check-') as temp:
        root = Path(temp)
        shutil.copyfile(A.collector(f'{kind}/bench_device.py'), root / 'bench_device.py')
        checker = root / kind
        checker.mkdir()
        if kind == 'app':
            for name in ('check.py', 'run.py'):
                shutil.copyfile(A.collector(f'app/{name}'), checker / name)
        else:
            for name in ['check.py', 'suite.py']:
                shutil.copyfile(REPO / 'tools/version-bench' / name, checker / name)
            shutil.copytree(REPO / 'tools/version-bench/suites', checker / 'suites')
        return subprocess.run([sys.executable, '-B', str(checker / 'check.py'), *map(str, args)],
                              capture_output=True, text=True)


def app_checks():
    directory = A.path('mi14/app')
    ...
    for source, snapshot in (('tools/app-bench/run.py', 'run.py'),
                             ('tools/app-bench/check.py', 'check.py'),
                             ('tools/bench_device.py', 'bench_device.py')):
        assert sources[source] == digest(A.collector(f'app/{snapshot}'))
    result = run_checker('app', directory)
    assert result.returncode in (0, 2), result.stderr
    verdict = json.loads(result.stdout)
    if (directory / 'summary.json').exists():
        assert verdict == read(directory / 'summary.json')

    previous = A.path('mi14/quality')
    # Revalidate the archived score/reference/export chain, not just its label.
    runpy.run_path(str(CHECKS / 'quality.py'))['quality_checks']()
    previous_app = read(previous / 'app/results.json')
    ...
    quality = read(previous / 'comet.json')
    ...
    return {'verdict': verdict, 'matched_scored_passes': matched, 'needs_scoring': unknown,
            ...
            'quality_source_sha256': digest(previous / 'comet.json')}


def native_checks(rel='mi14/native/results.json'):
    path = A.path(rel)
    report = read(path)
    if rel == 'mi14/native/results.json':
        assert report['protocol']['foreground_checks'] == 'awake-unlocked-focused-before-after-v1'
        for source, expected in report['collector_files_sha256'].items():
            assert digest(A.collector(f'native/{Path(source).name}')) == expected
    result = run_checker('native', path, path,
...
def native_table():
    ...
    report = read(A.path('mi14/native/results.json'))
    rows = []
...
if __name__ == '__main__':
    result = {}
    if A.path('mi14/native/results.json').exists():
        result['native'] = native_checks()
        result['native_rows'] = native_table()
    if A.path('mi14/app').exists():
