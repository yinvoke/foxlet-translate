#!/usr/bin/env python3
"""Summarize a versioned app suite; reject incomplete or noisy evidence."""
import argparse
import json
import math
from pathlib import Path
from statistics import median
from run import suite_scenarios, sha256, validate_result
...
def summarize(directory):
    ...
    try:
        scenarios = suite_scenarios(report.get("suite_id"))
    except ValueError as error:
        return {"suite_id": report.get("suite_id"), "accepted": False,
                "errors": [str(error)], "scenarios": []}
    ...
    if len(keys) != len(expected_cells) or set(keys) != expected_cells:
        errors.append(f"expected {len(expected_cells)} unique runs: all {len(scenarios)} scenarios x three rounds")
    grouped = {}
    temperatures = []
    for row in rows:
    ...
    return {"suite_id": report["suite_id"], "accepted": not errors, "errors": errors, "scenarios": summaries}
