...
本工具默认使用 **`android-app-v2`**，比较 Foxlet AAR 的单、双线程与 Google ML Kit，共六场景。后续常规回测不再测四线程，以缩短耗时；历史 `android-app-v1` 八场景定义及结果保持不变，可显式用 `--suite android-app-v1` 复现。本工具与[原生基准](../version-bench/README.md)分别记录，不把 App PSS 与 native RSS 混算。旧版 150 条语料的图表另存于[历史归档](../../benchmark/data/v0.1.0/README.md#与-google-ml-kit-的历史评测)。

报告字段和命令参数中的 `bergamot` 保留为协议中的后端标识，以兼容历史数据；应用显示名称为 Foxlet。

## 固定协议

v2 每个版本保留以下六行；v1 另含两个语向的四线程场景：
...
- FLORES-200 devtest 前 200 条，输入来自仓库 `benchmark/app/src/main/assets/bench/`；双方使用同一批源文。Foxlet 使用 AAR 批量接口，ML Kit 按 SDK 接口逐条调用。
...
- 新采集器追加 `foreground_checks: awake-unlocked-focused-before-after-v1`：每进程前后检查屏幕为 Awake、锁屏关闭、焦点属于基准 app，任一缺失或不满足即停止并保留结果。`am start` 返回成功不等于设备已解锁。这是运行条件的额外证据，不改所选协议的场景、重复次数或计时口径；只采集前后快照，不宣称连续监控全程前台状态。
...
调整场景清单、模型、语料或计时方法时新增协议或基线，并保留历史定义。常规采集只运行当前版本，标注与旧版的差异；明确请求配对实验后才新增双方测量。

单个 App 进程输出仍沿用 `android-app-v1` 的原始 JSON 格式和计时范围；`results.json` 的 `suite_id` 标识场景清单版本，检查器按该清单要求完整覆盖。

## 构建、准备与运行

每个版本只测当前 APK，旧版直接使用归档。README、版本对比和发布说明通过[统一生成器](../benchmark-report/README.md)读取原始结果，改变展示不触发本工具；缺项不会自动补测。
...
adb -s DEVICE install -r benchmark/app/build/outputs/apk/debug/benchmark-app-debug.apk

# 单独准备 ML Kit 模型；Foxlet 模型使用 benchmark-app 的 files/models 目录。
python3 tools/app-bench/run.py prepare /path/to/new-prepare \
  --adb /path/to/adb --serial DEVICE \
  --apk benchmark/app/build/outputs/apk/debug/benchmark-app-debug.apk
...
`quality` 模式仅导出所选协议的一轮完整场景的译文，不等待频率门槛；其计时不能晋升为正式性能结果。`prepare` 模式不产生质量或速度结论。

检查器要求 v2 完整 18 个进程（v1 为 24 个）、模型不变、有效 PSS 曲线、正确输出哈希和满足起跑门槛。起跑温度跨度超过 3°C、同场景三进程任一指标极差超过中位数 10%，或相同调用输出变化，均需复查，不能标为正式基线通过；补测须另有明确请求。退出码 0 表示通过，2 表示证据不完整或未通过稳定性检查；原始记录一律保留。
...
默认拒绝未完成的采集。如果设备断开，但所选协议的全部场景都已有完整的 200 条译文，可以显式追加 `--completed-quality-only`，仅为已完成调用计算质量分数。结果会列出原始报告的失败状态、已完成/计划进程数和所有缺项；它不补齐三轮性能结果，也不证明模型文件在整个场次中未变。
...
在设备上运行 `quality_reference workers config.yml [pivot-target.yml] < corpus.txt`，输出 200 个译文组成的 JSON 数组。worker 0 使用 blocking，正数使用 async；模型、语料、参数、二进制与源码指纹须与结果一同归档。该路径只提供旧版原生质量参考，不代表 AAR 端到端实测。新版辅助程序的输出还应与新版 AAR 对照，确认路径一致。
...
CI 使用合成数据验证检查逻辑；手机性能结论仅来自固定设备采集。正式结果按版本保存到 [benchmark/data/](../../benchmark/data/README.md)。
